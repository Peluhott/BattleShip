class GameBoard {
    ...
    placeShip(point, ship, orientation) { // its going to recieve a point, check what orientation, iterate for length, and place the ship on the board
        if (orientation === 'horizontal') {  // if the orientation is horizontal, the ship will be placed horizontally on the board
            ship.location[0][0] = point[0]; // sets first point of ship x value
            ship.location[0][1] = point[1]; // sets first point of ship y value

            for (let i = 1; i < ship.length; i++) {
                ship.location[i][0] = point[0] + i; // sets the next point of the ship x value
                ship.location[i][1] = point[1]; // sets the next point of the ship y value
            ...
            for (let i = 1; i < ship.length; i++) {
                ...
                ship.location[i][0] = point[0]; // sets the next point of the ship x value
                ship.location[i][1] = point[1] + i; // sets the next point of the ship y value
            }
        }
    }
